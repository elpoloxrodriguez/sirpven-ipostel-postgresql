...
del Estado Aragua, en fecha XXX, bajo el N° Xxx, tomo ${data.tomo_archivo_curp}; la cual prestara servicios al Operador Postal
xxxxxxxxxxxxx identificado con el RIF ${data.rif}, Código Postal N° ${data.concesion_postal_curp}, de conformidad a lo
...
supeditado, conforme a lo establecido en la Contrato de Concesión suscrito por su representada.`,
    ...
    doc.setFontSize(12);
    doc.setFont(undefined, "bold");
    doc.text(`AUTORIZACIÓN VALIDA DESDE: ${'01/01/' + this.anio} HASTA: ${'31/12/' + this.anio} `, 15, 255, { align: "justify" });
  ...
  GenerarFactura(data: any, MantenimientoYSeguridad: any) {
    ...
    for (let i = 0; i < MantenimientoYSeguridad.length; i++) {
      ...
      this.tempMantenimiento = [
       ...
       this.valor1 = element[1].tasa_petro * data[0].ListaFacturas[0].petro_dia
      this.tempSeguridad = [
        element[1].nombre_tipo_pagos,
        element[1].iniciales_tipo_pagos,
        this.utilService.ConvertirMoneda(this.valor1),
        '---',
        '---',
      ];
    }
    ...
    this.rowsFacturas.push(this.tempDataRowsFacturas, this.tempMantenimiento, this.tempSeguridad);
